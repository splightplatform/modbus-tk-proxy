...
class TcpMaster(Master):
    ...
    def _do_open(self):
        ...
        if self._sock:
            self._sock.close()
        if self._proxy:
            socks.setdefaultproxy(proxy_type=self._proxy.get('proxy_type', None), 
                                  addr=self._proxy.get('proxy_addr', None),
                                  port=self._proxy.get('proxy_port', None),
                                  rdns=self._proxy.get('proxy_rdns', None),
                                  username=self._proxy.get('proxy_username', None),
                                  password=self._proxy.get('proxy_password', None)
            )
    ...
    def proxy_set(self, **proxy_args):
        """Configure proxying of MODBUS TCP connection. Enables support for SOCKS or
        HTTP proxies.
        ...
        (Optional)
        proxy_port: Port number of proxy server (default: 1080)
